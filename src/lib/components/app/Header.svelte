...
>
    ...
    <header class="flex-grow">
        ...
        >
            ...
            >
                {#if $darkMode}
                    <img class="absolute left-0 top-0 bottom-0 right-0" draggable={false} src="/images/moon.svg"
                         alt="Dark mode Icon"/>
                {:else}
                    <img class="absolute  left-0 top-0 bottom-0 right-0" draggable={false} src="/images/sun.svg"
                         alt="Light mode Icon"/>
                {/if}
            </div>
        </div>
